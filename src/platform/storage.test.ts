...
describe('player storage adapter', () => {
  it('falls back when local data is malformed', () => {
    ...
    expect(loadPlayer()).toEqual({ lives: 5, bestScores: {}, unlockedLevels: {} })
  })
  ...
  it('clamps lives and rejects invalid score entries', () => {
    ...
    expect(loadPlayer()).toEqual({ lives: 9, bestScores: { safe: 120 }, unlockedLevels: { water: 12 } })
  })

  it('degrades safely when browser storage is unavailable', () => {
    vi.spyOn(Storage.prototype, 'setItem').mockImplementation(() => { throw new Error('denied') })
    expect(savePlayer({ lives: 3, bestScores: {}, unlockedLevels: {} })).toBe(false)
  })
})
