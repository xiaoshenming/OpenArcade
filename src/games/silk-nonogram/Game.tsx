...
import { CROSSED, EMPTY, ERROR, FILLED, formatClock, lineDone, puzzleComplete, scoreFor, type CellMark } from './logic'
import { createSilkPuzzle, getSilkLevel, SILK_LEVEL_COUNT, type SilkLevelSpec } from './levels'
import type { SilkPuzzle } from './logic'
...
function SilkWeave({ paused, audio, emit, levelNumber, spec, puzzle }: WeaveProps) {
  const [marks, setMarks] = useState<CellMark[]>(() => Array<CellMark>(puzzle.pattern.length).fill(EMPTY))
  ...
  return (
    <div className={`silk-game mode-${spec.mode}${done === 'win' ? ' is-solved' : ''}${done === 'lose' ? ' is-failed' : ''}`} aria-label="织锦谜图游戏区">
      ...
      <div className="silk-board" style={{ '--silk-size': spec.size } as CSSProperties}>
        ...
        <div className="silk-row-clues">
          {puzzle.rowClues.map((clue, row) => (
            <div key={row} className={`silk-clue${lineState.rows[row] ? ' is-done' : ''}`} aria-label={`第 ${row + 1} 行提示 ${clue.join(' ') || 0}`}>
              {(clue.length ? clue : [0]).map((run, at) => <b key={at}>{run}</b>)}
            </div>
          ))}
        </div>
        <div className="silk-grid">
          {marks.map((value, index) => (
            <button
              key={index}
              type="button"
              className={`silk-cell${cellClass(value)}`}
              onClick={() => mark(index)}
              disabled={!!done || paused}
              aria-label={`第 ${Math.floor(index / spec.size) + 1} 行第 ${index % spec.size + 1} 列：${value === FILLED ? '已填' : value === CROSSED ? '已打叉' : value === ERROR ? '失误格' : '未定'}`}
            />
          ))}
        </div>
      </div>
      <div className="silk-actions">
        ...
        <button type="button" className="silk-action" onClick={() => emit({ type: 'request-restart' })}><RotateCcw size={15} />重开本关</button>
      </div>
...
export default function SilkNonogramGame({ paused, muted, emit, level = 1 }: GameModuleProps) {
  ...
  return <SilkWeave key={levelNumber} paused={paused} audio={audio} emit={emit} levelNumber={levelNumber} spec={spec} puzzle={puzzle} />
}
