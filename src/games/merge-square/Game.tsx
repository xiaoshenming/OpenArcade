...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, EyeOff, LockKeyhole, Pin, RotateCcw } from 'lucide-react'
...
import { birthValueAt, createSpawner, hasAnyMove, maxValue, slideGrid, type Direction, type Grid, type Spawner } from './logic'
...
function MergeSquareRun({ levelNumber, spec, paused, emit, audio, onRestart }: RunProps) {
  ...
  const attemptMove = useCallback((direction: Direction) => {
    ...
    const spawned = spawnerRef.current?.spawn(nextGrid, index, idRef.current + 1)
    if (spawned) {
      idRef.current += 1
      nextGrid = spawned.grid
  ...
  return (
    <div className={`msq-game chapter-${spec.chapter} mode-${status}`} aria-label="合并方阵游戏区">
      ...
      <div className="msq-stage">
        <div className="msq-board" style={{ '--gap': '10px' } as CSSProperties}>
          <div className="msq-cells">{Array.from({ length: 16 }, (_, cell) => <span key={cell} />)}</div>
          <div className="msq-tiles">
            {grid.map((tile, index) => tile && (
              <div key={tile.id} className="msq-tile" style={{ '--x': index % 4, '--y': Math.floor(index / 4) } as CSSProperties}>
                <span className={`msq-face v-${Math.min(2048, tile.value)} ${tile.locked ? 'is-locked' : ''} ${tile.fixed ? 'is-fixed' : ''} ${popped.includes(tile.id) ? 'is-popped' : ''}`}>
                  {tile.value}
                  {tile.fixed && <Pin size={11} />}
        ...
        <div className="msq-budget"><span style={{ width: `${progress}%` } as CSSProperties} /></div>
      </div>
