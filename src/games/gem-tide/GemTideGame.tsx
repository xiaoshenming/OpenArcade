...
export default function GemTideGame({ paused, muted, emit, level = 1, sessionKey = 0 }: GameModuleProps) {
  const levelNumber = Math.min(LEVEL_COUNT, Math.max(1, Math.floor(level) || 1))
  useEffect(() => {
    emit({ type: 'ready' })
    emit({ type: 'started' })
  }, [emit])
  return <GemTideSession key={`${sessionKey}-${levelNumber}`} levelNumber={levelNumber} paused={paused} muted={muted} emit={emit as (event: GameEvent) => void} />
}
...
function GemTideSession({ levelNumber, paused, muted, emit }: { levelNumber: number; paused: boolean; muted: boolean; emit: (event: GameEvent) => void }) {
  ...
  const attemptSwap = (cell: number) => {
    ...
    const nextScore = Math.min(10000, score + result.gained)
    ...
    if (check.done) finish(Math.min(10000, nextScore + (spec.moves - nextMoves) * 25), 'won')
    else if (nextMoves >= spec.moves) finish(nextScore, 'lost')
  }
